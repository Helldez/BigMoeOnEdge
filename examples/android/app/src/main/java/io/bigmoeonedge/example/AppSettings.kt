...
enum class DenseWeights(val flag: String, val label: String, val blurb: String) {
    ...
    AHWB("ahwb", "Pinned (experimental)", "As Anon, but into dma-buf memory the kernel is not allowed to reclaim at all. Experimental: pinning does not create memory, so whatever the dense weights stop yielding is taken from the expert cache instead. Compare it against Anon before trusting it."),
}
