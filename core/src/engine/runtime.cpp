...
RunResult run(const RunConfig & cfg,
    ...
    int n_prompt = llama_tokenize(vocab, prompt.c_str(), (int) prompt.size(),
                                  tokens.data(), (int) tokens.size(), /*add_special*/ true, /*parse_special*/ true);
    if (n_prompt < 0) {
    ...
    for (int t = 0; t < cfg.n_predict; ++t) {
        ...
        TokenMetrics m;
        m.step = n_gen; m.steps = cfg.n_predict;
        m.wall_ms = wall * 1000.0;
        m.piece = delta; m.text = gen;
        if (cfg.moe.enabled) {
            ...
            m.compute_ms = m.wall_ms - m.io_ms; if (m.compute_ms < 0) m.compute_ms = 0;
